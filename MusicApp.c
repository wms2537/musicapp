...
#define DBG(...)                           \
    ...
    {                                      \
        if (debug_enabled)                 \
            app_log("DEBUG", __VA_ARGS__); \
    } while (0)

void app_log(const char *type, const char *format, ...)
{
    if (log_fp == NULL)
    { // Fallback if log file couldn't be opened
...
void set_volume_by_level_idx(int level_idx)
{
    ...
    if (level_idx < 0)
        level_idx = 0;
    if (level_idx >= NUM_VOLUME_LEVELS)
        level_idx = NUM_VOLUME_LEVELS - 1;
    ...
    if (target_raw_volume < vol_min)
        target_raw_volume = vol_min;
    if (target_raw_volume > vol_max)
        target_raw_volume = vol_max;
...
int main(int argc, char *argv[])
{
    ...
    if (!user_specified_format || pcm_format == SND_PCM_FORMAT_UNKNOWN)
    {
        ...
        case 8:
            pcm_format = SND_PCM_FORMAT_U8;
            break; // Common for 8-bit
        case 16:
            pcm_format = SND_PCM_FORMAT_S16_LE;
            break; // Common for 16-bit
        case 24:
            pcm_format = SND_PCM_FORMAT_S24_LE;
            break; // Common for 24-bit
        case 32:
            pcm_format = SND_PCM_FORMAT_S32_LE;
    ...
    while (1)
    {
        ...
        if (wav_header.bits_per_sample > 0)
        ...
        { // if WSOLA didn't set the buffer
            if (fabs(current_speed - 1.0) < 1e-6)
            {                                                                          // No speed change
...
{
    if (!state || !input_samples || num_input_samples <= 0)
        return;
...
/* ------- window generation ------------------------------------ */
static void generate_sine_window_q15(short *w,int N){
    if (N <= 0) return; // Basic safety for N > 0
    if (N == 1) { // sin(0) = 0 for N=1 (n=0 only)
        w[0] = 0;
        return;
    }
    for(int n=0;n<N;n++)
        w[n]=(short)lrint( sin(M_PI*n/(N-1)) * 32767.0 );
}
    ...
    if (!match_found || max_correlation_float < 0.1f)
    {
        ...
        if (consecutive_low_correlations > 2)
        {
            ...
            else
            {
                ...
                {
                    int candidate_idx = (ideal_search_center_ring_idx + test_offset + state->input_buffer_capacity) % state->input_buffer_capacity;
                ...
                int analysis_frame_ms_arg, float overlap_percentage_arg, int search_window_ms_arg)
{
    ...
    if (sample_rate_arg <= 0 || num_channels_arg <= 0 || initial_speed_factor_arg <= 0.0 ||
        ...
        search_window_ms_arg < 0)
    { // search_window_ms can be 0 for no search
    ...
    if (s->analysis_frame_samples == 1 && s->overlap_samples == 0)
        s->overlap_samples = 1;
    ...
    // Generate Sine (sqrt-Hanning) window directly into Q15 analysis_window_function
    generate_sine_window_q15(s->analysis_window_function, s->analysis_frame_samples);
